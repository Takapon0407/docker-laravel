...
1. Vite ビルド・`composer install`
2. SCP で EC2 の `/home/ec2-user/tektektech` に転送
3. `.env.production.enc` を復号して `.env` を生成
4. `docker-compose up -d --build` → `docker image prune -f`

`.env.production` の暗号化・復号は `openssl aes-256-cbc`（鍵: `ENCRYPTION_SECRET` シークレット）。
